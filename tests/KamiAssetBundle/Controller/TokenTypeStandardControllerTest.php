...
class TokenTypeStandardControllerTest extends ApiTestCase
{
    public function testIndexLoggedInAsAnonymous()
    {
        $response = $this->request('GET', '/api/token-type-standard');
    ...
    public function testFilterLoggedInAsAnonymous()
    {
        ...
        $response = $this->request('GET', '/api/token-type-standard/filter?filter=' . $filter);
        $this->assertJsonResponse($response, 403);
    }

    public function testCreateLoggedInAsAnonymous()
    {
        $response = $this->request('POST', '/api/token-type-standard', [
            'token_type_standard' => [
                'title' => 'test'
            ]
        ]);
    ...
    {
        $response = $this->requestByWorker('POST', '/api/token-type-standard', [
            'token_type_standard' => [
                'title' => 'test'
            ]
        ]);
    ...
    {
        $response = $this->request('PUT', '/api/token-type-standard/1', [
            'token_type_standard' => [
                'title' => 'test1'
            ]
        ]);
    ...
    {
        $this->logInAsAdmin();
        $response = $this->request('PUT', '/api/token-type-standard/1', ['token_type_standard' => ['titl' => 'edit']]);
        $this->assertJsonResponse($response, 400);
    }

    public function testDeleteLoggedInAsAnonymous()
    {
        $response = $this->request('DELETE', '/api/token-type-standard/1');
        $this->assertJsonResponse($response, 403);
    }
