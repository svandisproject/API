...
class SaleStageTest extends ApiTestCase
{
    public function testIndexLoggedInAsAnonymous()
    {
        $response = $this->request('GET', '/api/sale-stage');
    ...
    public function testFilterLoggedInAsAnonymous()
    {
        ...
        $response = $this->request('GET', '/api/sale-stage/filter?filter=' . $filter);
        $this->assertJsonResponse($response, 403);
    }

    public function testCreateLoggedInAsAnonymous()
    {
        $response = $this->request('POST', '/api/sale-stage', [
            'sale_stage' => [
                'title' => 'test'
            ]
        ]);
    ...
    {
        $response = $this->requestByWorker('POST', '/api/sale-stage', [
            'sale_stage' => [
                'title' => 'test'
            ]
        ]);
    ...
    {
        $response = $this->request('PUT', '/api/sale-stage/1', [
            'sale_stage' => [
                'title' => 'test2'
            ]
        ]);
    ...
    {
        $response = $this->request('DELETE', '/api/sale-stage/1');
        $this->assertJsonResponse($response, 403);
    }
